...
> **Refined 2026-09-04, not overturned.** Everything below stands for the
> 200000 step budget it was measured at. At 1000000 steps depth 2 does not
> stay degenerate: it converges on loss ln(6), having solved the D4 factor
> exactly and learned nothing of S3. So "not learnable above depth 1" is
> true of the answer and false of the computation, and the wall is specific
> to one factor rather than to depth. See "One factor is learnable alone
> and the other is not" and "S3 is never computed" in section 4.
>
> Point 4 below, refuting the order-independent bit hypothesis, was itself
> later corrected: those models were degenerate and the probe was
> structurally blind on them, which the entry already flags in the next
> paragraph. See the superseded abelian quotient entry.
...
### SUPERSEDED 2026-09-04. A loop-count curve exists on one seed, milestone 3, 2026-09-04

> **Superseded within hours by "H1 SUPPORTED", which has all three seeds
> and the breadth arm.** Kept because its threshold-sensitivity table is
> the more careful treatment of how much k_min depends on tau, and because
> the three-seed result changed the per-seed numbers enough that seeing
> both is the honest record: on seed 0 alone k_min read 1,1,1,2,2,2, and
> across three seeds the same threshold gives 1,2,2,2,3,4.
