...
- **Stimulus.** An asymmetric glyph with trivial stabiliser, so every group element gives a visually distinct state, rendered on the canvas, plus a strip of `n` operator glyphs rendered along the bottom rows, in order. **The L-tromino suggested in earlier drafts does not work**: it is symmetric under diagonal reflection, and that reflection paired with the transposition of its two arm colours fixes the glyph, giving a stabiliser of order 2 and only 24 distinct states rather than 48. The glyph in use is a chiral J-tetromino with three distinctly coloured cells and one neutral cell, whose stabiliser is asserted trivial computationally in `tests/test_groups.py`. See `docs/decisions.md` D-017.
...
- **Anti-shortcut sampling.** Sample the operator sequence so that every prefix product is uniform over the group. Concretely: draw the target final element uniformly, then draw a uniformly random factorisation of length `n`. This kills prefix-statistics shortcuts, and it is what `random_factorisation` in `groups.py` implements. It guarantees three things, all tested: labels uniform over 48 classes, every proper prefix product uniform, and every single-operator marginal uniform including the solved-for last position. It does **not** make the operator multiset uninformative, and nothing can; see gate check 3 in Section 4.6.
...
3. The bag-of-operators shortcut probe must not exceed the **order-blind ceiling**, and that ceiling must sit below the M1 threshold tau by a clear margin at every depth in use. **This replaces an earlier criterion requiring the probe to sit at chance, which is mathematically unachievable**: at depth 2 an order-blind predictor chooses between `a.b` and `b.a` and scores about 0.65 against a chance level of 0.021. Measured ceilings are 0.652, 0.477, 0.375, 0.301 and 0.256 at depths 2 to 6. They never approach tau = 0.90, which is why `k_min` to threshold still isolates sequential composition. See `docs/decisions.md` D-016 and the ceiling table in `docs/findings.md`.
